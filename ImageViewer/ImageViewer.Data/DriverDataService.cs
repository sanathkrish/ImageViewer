...
namespace ImageViewer.Data
{
    public class DriverDataService
    {
        ...
        public async Task<bool> UpdateFileDeriver(DriverInfo driver) 
        {
            ...
            command.CommandText = @"
                UPDATE Drivers
                SET Name = @name, Path = @path, type = @type, DateAdded = @dateAdded, FreeSpace = @freeSpace, TotalSize = @totalSize
                WHERE Id = @id";
        ...
        public async Task<List<DriverInfo>> GetAllFileDriversAsync()
        {
            ...
            _connection.Open();
            var command = _connection.CreateCommand();
            command.CommandText = "SELECT Id, Name, Path, type, DateAdded, TotalSize, freespace FROM drivers";
            ...
            while (await reader.ReadAsync())
            {
                drivers.Add(new DriverInfo
                {
                    ...
                    FreeSpace = reader.GetInt64(6)
                });
            }
            return drivers;
        }

        public async Task<DriverInfo> GetFileDriversAsync(string path)
        {
            var drivers = new List<DriverInfo>();
            _connection.Open();
            var command = _connection.CreateCommand();
            command.CommandText = "SELECT Id, Name, Path, type, DateAdded, TotalSize, freespace FROM drivers where  lower(path) = lower(@path)";
